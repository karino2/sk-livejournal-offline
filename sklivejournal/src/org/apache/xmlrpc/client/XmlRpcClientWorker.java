...
/**
 * Object, which performs a request on the clients behalf. The client maintains
 * a pool of workers. The main purpose of the pool is limitation of the maximum
...
public class XmlRpcClientWorker implements XmlRpcWorker {
	...
	public Object execute(XmlRpcRequest pRequest) throws XmlRpcException {
		try {
			...
			return client.getTransportFactory().getTransport().sendRequest(
					pRequest);
		} finally {
	...
	public void execute(final XmlRpcRequest pRequest,
		...
		Runnable runnable = new Runnable() {
			public void run() {
				...
				try {
					XmlRpcClient client = (XmlRpcClient) getController();
					result = client.getTransportFactory().getTransport()

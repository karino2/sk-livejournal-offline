...
public class HttpUtil {
	 ...
	 * 
	 * @param pUser
	 *            User name, or null, if no Basic Authentication is being used.
	 * @param pPassword
	 *            Users password, or null, if no Basic Authentication is being
	 *            used.
	 * @param pEncoding
	 *            Encoding being used for conversion of the credential string
	 *            into a byte array.
	 * @return Base64 encoded credentials, for use in the HTTP header
	 * @throws UnsupportedEncodingException
	 *             The encoding <code>pEncoding</code> is invalid.
	 */
	public static String encodeBasicAuthentication(String pUser,
			String pPassword, String pEncoding)
			throws UnsupportedEncodingException {
		if (pUser == null) {
			return null;
		}
	 ...
	 * 
	 * @param pHeaderValue
	 *            The HTTP header value being parsed. This is typically the
	 *            value of "Content-Encoding", or "Accept-Encoding".
	...
	public static boolean isUsingGzipEncoding(String pHeaderValue) {
		...
		for (StringTokenizer st = new StringTokenizer(pHeaderValue, ","); st
				.hasMoreTokens();) {
	...
	/**
	 * Returns, whether the HTTP header value <code>pHeaderValue</code>
	 * indicates, that another encoding than "identity" is used. This is
	 * typically the value of "Transfer-Encoding", or "TE".
	 * 
	 * @return Null, if the transfer encoding in use is "identity". Otherwise,
	 *         another transfer encoding.
	 */
	...
	/**
	 * Returns, whether the HTTP header values in <code>pValues</code> indicate,
	 * that GZIP encoding is used or may be used.
	 * 
	 * @param pValues
	 *            The HTTP header values being parsed. These are typically the
	 *            values of "Content-Encoding", or "Accept-Encoding".
	...
	/**
	 * Reads a header line from the input stream <code>pIn</code> and converts
	 * it into a string.
	 * 
	 * @param pIn
	 *            The input stream being read.
	 * @param pBuffer
	 *            A buffer being used for temporary storage. The buffers length
	 *            is a limit of the header lines length.
	 * @return Next header line or null, if no more header lines are available.
	 * @throws IOException
	 *             Reading the header line failed.
	 */
	public static String readLine(InputStream pIn, byte[] pBuffer)
			throws IOException {
		int next;
		int count = 0;
		while (true) {
	 ...
	 * 
	 * @param pConfig
	 *            The request configuration being created.
	 * @param pLine

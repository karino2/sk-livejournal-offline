...
public class ReflectionUtil {
	/**
	 * This method attempts to set a property value on a given object by calling
	 * a matching setter.
	 * 
	 * @param pObject
	 *            The object, on which a property is being set.
	 * @param pPropertyName
	 *            The property name.
	 * @param pPropertyValue
	 *            The property value.
	 * @throws IllegalAccessException
	 *             Setting the property value failed, because invoking the
	 *             setter raised an {@link IllegalAccessException}.
	 * @throws InvocationTargetException
	 *             Setting the property value failed, because invoking the
	 *             setter raised another exception.
	 * @return Whether a matching setter was found. The value false indicates,
	 *         that no such setter exists.
	 */
	public static boolean setProperty(Object pObject, String pPropertyName,
			String pPropertyValue) throws IllegalAccessException,
			InvocationTargetException {
		final String methodName = "set"
				+ pPropertyName.substring(0, 1).toUpperCase()
				...
				if (parameterType.equals(boolean.class)
					...
					if (pPropertyValue.length() != 1) {
						throw new IllegalArgumentException(
								"Invalid value for parameter " + pPropertyName
										+ "(length != 1):" + pPropertyValue);
					}
					...
					throw new IllegalStateException("The property "
							+ pPropertyName + " has an unsupported type of "
							+ parameterType.getName());
				}
			} catch (NumberFormatException e) {
				throw new IllegalArgumentException(
						"Invalid value for property " + pPropertyName + ": "
								+ pPropertyValue);
			}

...
/**
 * This class is responsible for writing an XmlRpc request or an XmlRpc response
 * to an output stream.
 */
public class XmlRpcWriter {
	...
	public void write(XmlRpcRequestConfig pConfig, Object pResult)
		...
		handler.startElement("", "methodResponse", "methodResponse",
				ZERO_ATTRIBUTES);
		handler.startElement("", "params", "params", ZERO_ATTRIBUTES);
		handler.startElement("", "param", "param", ZERO_ATTRIBUTES);
		writeValue(pResult);
	 ...
	 *            The error message
	 * @param pThrowable
	 *            An exception, which is being sent to the client
	 * @throws SAXException
	 *             Writing the error message failed.
	 */
	public void write(XmlRpcRequestConfig pConfig, int pCode, String pMessage,
		...
		if (pThrowable != null
				&& extensions
				&& (pConfig instanceof XmlRpcStreamRequestConfig)

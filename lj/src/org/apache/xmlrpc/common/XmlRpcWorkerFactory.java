...
public abstract class XmlRpcWorkerFactory {
	 ...
	 * 
	 * @return An instance of {@link XmlRpcWorker}, which is ready for use.
	...
	public synchronized XmlRpcWorker getWorker() throws XmlRpcLoadException {
		...
		if (max > 0 && numThreads == max) {
			throw new XmlRpcLoadException(
					"Maximum number of concurrent requests exceeded: " + max);
		}
		if (max == 0) {
			return singleton;
		}
	...
	/**
	 * Called, when the worker did its job. Frees resources and decrements the

...
public class XmlRpcLocalTransport extends XmlRpcTransportImpl {
	...
	private boolean isExtensionType(Object pObject) {
		if (pObject == null) {
			...
			for (Iterator iter = ((Collection) pObject).iterator(); iter
					.hasNext();) {
			...
			return !(pObject instanceof Integer || pObject instanceof Date
					|| pObject instanceof String || pObject instanceof byte[] || pObject instanceof Double);
		}
	}
	...
	public Object sendRequest(XmlRpcRequest pRequest) throws XmlRpcException {
		...
		if (!config.isEnabledForExtensions()) {
			for (int i = 0; i < pRequest.getParameterCount(); i++) {
				if (isExtensionType(pRequest.getParameter(i))) {
					throw new XmlRpcExtensionException(
							"Parameter "
									+ i
									+ " has invalid type, if isEnabledForExtensions() == false");
				}
			}
		}
		final XmlRpcRequestProcessor server = ((XmlRpcLocalClientConfig) config)
				.getXmlRpcServer();
		Object result;
		try {
			...
			throw new XmlRpcClientException("Failed to invoke method "
					+ pRequest.getMethodName() + ": " + t.getMessage(), t);
		}
		if (!config.isEnabledForExtensions()) {
			if (isExtensionType(result)) {
				throw new XmlRpcExtensionException(
						"Result has invalid type, if isEnabledForExtensions() == false");
			}
		}
		...
		final TypeConverterFactory typeConverterFactory = server
				.getTypeConverterFactory();
		final TypeConverter typeConverter = typeConverterFactory
				.getTypeConverter(result.getClass());

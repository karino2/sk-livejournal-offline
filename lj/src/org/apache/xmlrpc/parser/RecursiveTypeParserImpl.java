...
public abstract class RecursiveTypeParserImpl extends TypeParserImpl {
	...
	protected RecursiveTypeParserImpl(XmlRpcStreamConfig pConfig,
			NamespaceContextImpl pContext, TypeFactory pFactory) {
		cfg = pConfig;
		context = pContext;
		factory = pFactory;
	}
			...
			if (typeParser == null) {
				...
				if (typeParser == null) {
					if (XmlRpcWriter.EXTENSIONS_URI.equals(pURI)
							&& !cfg.isEnabledForExtensions()) {
						String msg = "The tag "
								+ new QName(pURI, pLocalName)
								+ " is invalid, if isEnabledForExtensions() == false.";
						throw new SAXParseException(msg, getDocumentLocator(),
								new XmlRpcExtensionException(msg));
					} else {
						throw new SAXParseException("Unknown type: "
								+ new QName(pURI, pLocalName),
								getDocumentLocator());
					}
				}
				...
				if (text.length() > 0) {
					typeParser.characters(text.toString().toCharArray(), 0,
							text.length());
					text.setLength(0);
				}
			}

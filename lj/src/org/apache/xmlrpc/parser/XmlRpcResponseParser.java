...
public class XmlRpcResponseParser extends RecursiveTypeParserImpl {
	 ...
	 * @param pTypeFactory
	 *            The type factory for creating instances of {@link TypeParser}.
	 */
	...
	protected void addResult(Object pResult) throws SAXException {
		if (isSuccess) {
			...
			if (faultCode == null) {
				throw new SAXParseException("Missing faultCode",
						getDocumentLocator());
			}
			try {
		...
		case 0:
			if (!"".equals(pURI) || !"methodResponse".equals(pLocalName)) {
				throw new SAXParseException(
						"Expected methodResponse element, got "
			...
			if (isSuccess) {
				if (!"".equals(pURI) || !"param".equals(pLocalName)) {
					throw new SAXParseException("Expected param element, got "
							+ new QName(pURI, pLocalName), getDocumentLocator());
				}
			} else {
	...
	public void endElement(String pURI, String pLocalName, String pQName)
		...
		case 1: {
			...
			if (!"".equals(pURI) || !tag.equals(pLocalName)) {
				throw new SAXParseException("Expected /" + tag
						+ " element, got " + new QName(pURI, pLocalName),
						getDocumentLocator());
			}
			break;
		}
	...
	/**
	 * Returns whether the response returned success. If so, the result object
	 * may be fetched using {@link #getResult()}. Otherwise, you may use the
	 * methods {@link #getErrorCode()} and {@link #getErrorMessage()} to check
	 * for error reasons.
	 * 
